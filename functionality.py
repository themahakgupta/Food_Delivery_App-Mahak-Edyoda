...
def Register(type,user_json_file,admin_json_file,Full_Name,Address,Email,Password,Phone_Number):
...
def Login(type,user_json_file,admin_json_file,Email,Password):
...
def Create_Product(owner,product_json_file,product_ID,item_name,quantity_item,price,discount,total_stock_available):
...
def Read_Products(owner,item_json_file):
    '''Reading Products created by the admin(owner)'''
    fp=open(item_json_file,'r+')
    content=json.load(fp)
    productlist=[]
    for i in range(len(content)):
        if content[i]['Created By']==owner:
            productlist.append(content[i])
    fp.close()
    return productlist

def Read_Product_By_ID(item_json_file,Product_ID,Details):
...
def Update_Product(item_json_file,product_ID,detail_to_be_updated,new_value):
...
def Delete_Product(item_json_file,product_ID):
...
def Update_Member(user_json_file,name,detail_to_be_updated,new_value):
...
def Place_Order(order_json_file,ordered_by,delivery_address,item_json_file,product_ID,Quantity,Order_ID):
